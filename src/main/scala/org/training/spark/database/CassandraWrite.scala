...
object CassandraWrite {
  ...
  def main(args: Array[String]) {
    ...
    val cassandraOptions = Map( "table" -> "sales1", "keyspace" -> "ecommerce")
   ...
   //usersdf.registerTempTable("users")
    ...
    salesDf.select(salesDf("customerId").alias("customerid"), salesDf("itemId").alias("itemid")).write
        .format("org.apache.spark.sql.cassandra")
        .options(cassandraOptions).option("confirm.truncate","true").mode(SaveMode.Overwrite).save()

    /* Cassandra commands
    create keyspace ecommerce with replication = { 'class':'SimpleStrategy', 'replication_factor':1} ;
    create table ecommerce.sales1(customerId int, itemId string);
     */
  }
}
