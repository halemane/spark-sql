...
object HiveTableCreation {
  ...
  def main(args: Array[String]) {
    ...
    //sales.write.insertInto("learning.sales")
    ...
    //sales.write.mode(SaveMode.Overwrite).saveAsTable("sales")
    //hiveContext.sql("create table sales as select * from test")
    //hiveContext.sql("show tables").show()
    hiveContext.table("sales").show()
